...
- [x] Phase 3 — Projet dbt-core (pas dbt-fusion, qui exige une licence cloud) créé dans `src/dbt/hr_dwh/`. Source `raw.hr_employees` déclarée avec tests `not_null`/`unique` sur `EmpID` (via `quote: true`, contournement nécessaire car Postgres est sensible à la casse sur identifiants entre guillemets). Modèle `stg_employees` complet : colonnes renommées en snake_case, tous les champs texte trimés, dates parsées en `DATE`, `MarriedID`/`Termd` convertis en `is_married`/`is_terminated` (booléens), `Zip` reconstitué à 5 chiffres avec `lpad`. `dbt run` + `dbt test` passent.
- [x] Phase 4 — Couche `marts` complète : 9 dimensions (`dim_department`, `dim_position`, `dim_manager`, `dim_performance`, `dim_employment_status`, `dim_marital_status`, `dim_recruitment_source`, `dim_date`, `dim_employee`) + `fact_employee` (grain 1 ligne/employé, 311 lignes). 31 tests dbt passent (unicité + relations FK↔dimensions).
  - **Défauts de qualité découverts et traités** : `dept_id`/`perf_score_id`/`emp_status_id` ont chacun 1-2 lignes mal saisies dans le CSV source → résolu par vote majoritaire (`ROW_NUMBER() OVER (PARTITION BY id ORDER BY count DESC)`).
  - `position_id=13` et `position_id=23` couvrent chacun plusieurs intitulés de poste réels sans majorité claire → limitation documentée en commentaire SQL, non "corrigée" arbitrairement.
